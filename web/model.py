...
class CycleGAN():
    def __init__(self, lr_D=2e-4, lr_G=2e-4, image_shape=(256*1, 256*1, 3),
        ...
        self.identity_mapping_modulus = 20
        ...
        self.use_multiscale_discriminator = True
    ...
    def train(self, epochs, batch_size=1, save_interval=1):
        def run_training_iteration(loop_index, epoch_iterations):
            ...
            if loop_index % 20 == 0:
                ...
                self.save_tmp_images(real_images_A, real_images_B,
...
# Save and load
    ...
    def load_model_and_generate_synthetic_images(self):
        ...
        def save_image(image, name, domain):
            if self.channels == 1:
                image = image[:, :, 0]
            toimage(image, cmin=-1, cmax=1).save(os.path.join('generate_images',
                                                              'synthetic_images', domain, name))
            # Test B images
